...
CREATE TABLE da_inserire_in_clavis AS
  SELECT t.owner_library_id,t.inventory_serie_id,t.inventory_number,
   ...
   ci USING(owner_library_id,inventory_number,inventory_serie_id) WHERE ci IS NULL;
...
INSERT INTO clavis.item(
   ...
   date_created,date_updated,custom_field1,custom_field2
   )
   (select
	...
	titolo
     END as titolo,
     'F',0,'',
     date_created,date_updated,note,'Inserito in ToBi da ' || login
