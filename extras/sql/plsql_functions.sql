...
FOR i IN 1 .. array_upper(loan_date_begin, 1)
  LOOP
    ...
    tmpvar = CONCAT('Preso in prestito per ', DATE_PART('day', loan_date_end[i] - loan_date_begin[i]), ' giorni, dal ', to_char(loan_date_begin[i], 'DD-MM-YYYY'), ' al ', to_char(loan_date_end[i], 'DD-MM-YYYY'));
