...
-- DROP VIEW digitalizzati;

CREATE OR REPLACE VIEW digitalizzati AS

select bib_level,bib_type,created_by,modified_by,bid_source, bid, manifestation_id, title
  from manifestation cm join
  (select attachable_id as manifestation_id from public.attachments
