...
class SbctOrdersController < ApplicationController
  ...
  def show
    if SbctTitle.user_roles(current_user).include?('AcquisitionManager') and !@sbct_order.inviato?
      ...
      @sbct_order.autocreate_list(current_user)
    else
      user_session[:current_order]=nil
    ...
    @sbct_budget = @sbct_order.sbct_budget
    @prezzo_totale = SbctItem.somma_prezzo(@sbct_items).round(2)
    # render text:@sql and return
    respond_to do |format|
      format.html {}
      format.xls {
        data = SbctItem.create_order_file(@sbct_supplier, @sbct_items, :xls, @sbct_budget)
      ...
      format.csv {
        data = SbctItem.create_order_file(@sbct_supplier, @sbct_items, :csv, @sbct_budget)
