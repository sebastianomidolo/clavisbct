  ...
  fields.keys.each do |k|
    ...
    if as_value == :date
      ...
      if k == :talking_book_reader_id
        f=form.association :talking_book_reader, collection:TalkingBookReader.where('attivo').order(:cognome), include_blank:true, label:label
      else
        if k == :lettore
...
def talking_book_opac_presentation(clavis_manifestation,authorized)
  ...
  if !record.nil? and !access_control_key.blank? and authorized
    ...
    lnk2="https://#{request.host_with_port}/" + prenota_talking_book_path(record, :mid => mid, :dng_user => params[:dng_user], :ac => access_control_key)
    res << link_to(content_tag(:span, 'Richiedi prenotazione del CD', class: "badge"), lnk2) if params[:dng_user]=='sebastiano'
...
def talking_book_view_mp3_files(record)
  ...
  res=[]
  if record.first_mp3_filename.blank?
    # res << "Primo mp3: #{record.first_mp3_filename}\n"
  end
  if File.exists?(record.zip_filepath)
    ...
    res << "Per Seba: record.first_mp3_filename: #{record.first_mp3_filename} per record #{record.id}"
    res << link_to("<b>[Cancella file zip]</b>".html_safe, delete_zip_talking_book_path(record), confirm:'Confermi cancellazione file zip?')
  else
    res << "file zip audio non presente"
    if record.first_mp3_filename.blank?
      res << "Per Seba: record.first_mp3_filename.blank qui per record #{record.id}"
      res << link_to("<b>[Genera file zip</b>]".html_safe, make_zip_talking_book_path(record), confirm:'Confermi generazione file zip?')
    else
      res << "Per Seba: record.first_mp3_filename: #{record.first_mp3_filename} per record #{record.id}"
      res << link_to("<b>[Genera file zip</b>]".html_safe, make_zip_talking_book_path(record), confirm:'Confermi generazione file zip?')
    end
  ...
  def talking_books_breadcrumbs
    ...
    if params[:controller]=='talking_book_downloads'
      links << link_to('Catalogo dei libri parlati', talking_books_path)
      links << link_to('Admin', check_talking_books_path)
      links << link_to('Downloads', talking_book_downloads_path) if params[:action]=='show'
    end

    %Q{&nbsp; / &nbsp;#{links.join('&nbsp; / &nbsp;')}}.html_safe
  end

  def tb_downloads_index(records)
    res=[]
    res << content_tag(:tr, content_tag(:td, 'Data', class:'col-md-2') +
                            content_tag(:td, 'Utente', class:'col-md-1') +
                            content_tag(:td, 'HttpStatus', class:'col-md-1') +
                            content_tag(:td, 'Method', class:'col-md-1') +
                            content_tag(:td, 'Collocazione', class:'col-md-2') +
                            content_tag(:td, 'HttpPath', class:'col-md-2'), class:'success')


    
    records.each do |r|
      lnktxt = r.date.nil? ? '???' : r.date
      res << content_tag(:tr, content_tag(:td, link_to(lnktxt, talking_book_download_path(r.id))) +
                              content_tag(:td, r.username) +
                              content_tag(:td, r.http_status) +
                              content_tag(:td, r.method) +
                              content_tag(:td, r.collocazione) +
                              content_tag(:td, r.http_path))
    end
    content_tag(:div, content_tag(:table, res.join("\n").html_safe, :class=>'table table-striped'),
                :class=>'table-responsive')
  end

  def tb_download_show(record)
    res=[]
    record.attributes.keys.each do |k|
      next if record[k].blank?
      tv = record[k]
  ...
  def tb_download_title_info(record)
    "Collocazione: #{record.n}\nTitolo: #{record.titolo}"
  end
