# coding: utf-8
# Classe non usata - sostituita da Location
class SchemaCollocazioniCentrale < ActiveRecord::Base
  self.table_name='schema_collocazioni_centrale'
  attr_accessible :piano, :scaffale, :palchetto, :filtro_colloc, :bib_section_id, :notes, :locked, :library_id
  belongs_to :bib_section
  belongs_to :clavis_library, foreign_key:'library_id'
  before_save :check_record

  # validates :scaffale, presence: true

  
  # Commentato 30 maggio 2023:
  ...
  def update_centrale_locations
    # Non eseguo più come after_save da 30 maggio 2023
    self.clavis_items_import
  ...
  def items_count
    cond = []
    cond << self.conditions_for_update_centrale_locations(true,'cl')
  ...
  def conditions_for_update_centrale_locations(ignore_locked=false,table_alias='cl')
    tbl = table_alias.blank? ? '' : "#{table_alias}."
    conditions = []
    if !scaffale.nil?
      from,to=scaffale.split('-')
      if from.to_i == 0
        conditions << (to.nil? ? "#{tbl}primo_elemento = '#{from}'" : "#{tbl}primo_elemento between '#{from}' and '#{to}'")
      else
        conditions << (to.nil? ? "#{tbl}scaffale = #{from}" : "#{tbl}scaffale between #{from} and #{to}")
      end
    end
    if !palchetto.nil?
      r=[]
      ...
      if (palchetto =~ /^~/)
        conditions << "#{tbl}secondo_elemento #{palchetto}"
      else
        conditions << (r.size==1 ? "#{tbl}secondo_elemento = #{r.join}" : "#{tbl}secondo_elemento IN(#{r.join(',')})")
      end
    end
    if !filtro_colloc.nil?
      conditions << "(#{filtro_colloc})"
    end
    conditions << "#{tbl}piano is null" if !ignore_locked && !self.locked?
    conditions.join(' and ')
  end
  ...
  def sql_for_update_centrale_locations
    ...
    #"UPDATE clavis.centrale_locations as cl SET piano='#{self.bib_section.name}' WHERE #{self.conditions_for_update_centrale_locations};"

    %Q{UPDATE clavis.centrale_locations as cl SET piano=bs.name FROM public.schema_collocazioni_centrale sc, public.bib_sections bs
      WHERE #{self.conditions_for_update_centrale_locations}
        AND cl.library_id=sc.library_id AND sc.id=#{self.id} AND bs.id=sc.bib_section_id;}
  end
  ...
  def clavis_items_import
    cond1 = "WHERE primo_elemento is null and library_id=#{self.library_id}"
    cond2 = "WHERE secondo_elemento is null and library_id=#{self.library_id}"
    cond3 = "WHERE terzo_elemento is null and library_id=#{self.library_id}"
    sql=%Q{
     INSERT INTO clavis.centrale_locations(library_id,item_id,collocazione)
      (select ci.home_library_id,ci.item_id,cc.collocazione
        from clavis.collocazioni cc join clavis.item ci using(item_id)
        where ci.home_library_id = #{self.library_id} and ci.owner_library_id != -1)
      ON CONFLICT(item_id) DO NOTHING;
     UPDATE clavis.centrale_locations SET collocazione=replace(collocazione,' ', '.') WHERE primo_elemento is null and library_id = #{self.library_id};
     UPDATE clavis.centrale_locations SET primo_elemento = substr(split_part(collocazione,'.',1),1,128) #{cond1};
     UPDATE clavis.centrale_locations SET secondo_elemento = substr(split_part(collocazione,'.',2),1,128) #{cond2};
     UPDATE clavis.centrale_locations SET terzo_elemento = substr(split_part(collocazione,'.',3),1,128) #{cond3};
     UPDATE clavis.centrale_locations SET catena=split_part(substr(terzo_elemento,1,7),'/',1)::integer WHERE catena is null
                      AND library_id=#{self.library_id} AND split_part(substr(terzo_elemento,1,7),'/',1) ~ '^\\d+$';
     UPDATE clavis.centrale_locations SET catena=split_part(substr(secondo_elemento,1,7),'/',1)::integer WHERE catena is null
                      AND library_id=#{self.library_id} and split_part(substr(secondo_elemento,1,7),'/',1) ~ '^\d+$';}
    puts sql
  ...
  def sql_for_select_from_centrale_locations
    ...
    "SELECT * FROM clavis.centrale_locations cl WHERE #{sql} ORDER BY espandi_collocazione(collocazione) LIMIT 5;"
  end
  ...
  def SchemaCollocazioniCentrale.trova_piano(collocazione,library_id)
