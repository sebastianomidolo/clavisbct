...
task :bio_iconografico => :environment do
  ...
  def insert_into_digital_objects
    ...
    ActiveRecord::Base.connection.execute(sql).to_a.each do |r|
      ...
      d.save if d.changed?
      c=BioIconograficoCard.find(d.id)
      c.tags={}.to_xml(root:'r',:skip_instruct => true, :indent => 0)

      card_keys.each_pair do |k,v|
        next if r[v].blank?
        cont=r[v].strip
        m="#{v}="
        if c.respond_to?(m)
      ...
      c.lettera='IMPORTATI_DA_EXCEL'
      c.save
      t=c.bio_iconografico_topic
      if t.nil?
        puts "#{cnt} id #{c.id} => INTESTAZIONE MANCANTE"
        next
      else
        puts "#{cnt} id #{c.id} => #{c.intestazione} (#{t.inspect})"
      end

      topic_keys.each_pair do |k,v|
        next if r[v].blank?
        cont=r[v].strip
        m="#{v}="
        if t.respond_to?(m)
  ...
  fdout.write(%Q{DROP TABLE public.bioiconografico;CREATE TABLE public.bioiconografico (id integer, seqnum integer,
       "intestazione" varchar(1240), "luogo_nascita" varchar(80), "data_nascita" varchar(80),
       "data_nascita_stringa" char(9), "luogo_morte" varchar(80), "data_morte" varchar(80), 
       "data_morte_stringa" char(9), "luoghi_di_soggiorno" text,
       "esistenza_in_vita" varchar(80), "qualificazioni" text,
  ...
  excel_filename="/home/storage/preesistente/bct/BioIconografico/dataindex.xls"
  ...
  fdout.write(%Q{COPY public.bioiconografico ("id", "seqnum", "intestazione", "luogo_nascita", "data_nascita",
    "data_nascita_stringa", "luogo_morte", "data_morte", "data_morte_stringa", "luoghi_di_soggiorno",
    "esistenza_in_vita", "qualificazioni",
